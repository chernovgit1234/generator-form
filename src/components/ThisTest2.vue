<template>
  <div class="ready-form">
    <div class="ready-form__body" v-if="schemaFieldsTest2.length">
      <form ref="form" @submit.prevent="submitHandler()" class="ready-form__content">
        <div v-for="(field, i) in schemaFieldsTest2" :key="i" class="ready-form__field">
        <component
          :styles="field.style"
          :is="field.typeName"
          :classb="field.class"
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      this.$store.dispatch('createFormForTest2')
    },
    methods: {
      clickButton() {
    ...
    computed: {
      ...
      schemaFieldsTest2() {
        return this.$store.getters.schemaFieldsTest2
      }
    },
  }
</script>
